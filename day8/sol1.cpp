...
int countValues(std::vector< std::vector<char> > board, char signToCount);
	...
	for (auto i = signPositions.begin(); i != signPositions.end(); i++) {
		for (Coordinate firstCoord : i->second) {
			for (Coordinate secondCoord : i->second) {
				...
				Coordinate anitnodeCoord = firstCoord + diff;
				Coordinate anitnodeCoord1 = secondCoord - diff;
				if (anitnodeCoord.row >= 0 && anitnodeCoord.col >= 0
					&& anitnodeCoord.row < maxRows && anitnodeCoord.col < maxCols) {
					(*board)[anitnodeCoord.row][anitnodeCoord.col] = '#';
				}
				if (anitnodeCoord1.row >= 0 && anitnodeCoord1.col >= 0
                	&& anitnodeCoord1.row < maxRows && anitnodeCoord1.col < maxCols) {
                	(*board)[anitnodeCoord1.row][anitnodeCoord1.col] = '#';
                }
			}
		}
	}
